...
function getWhatsAppDmPolicy(state: AppViewState): string {
  const cfg = state.configSnapshot?.config ?? state.configForm;
  const wa = (cfg as any)?.channels?.whatsapp ?? (cfg as any)?.whatsapp;
...
function renderTopbarWhatsAppButton(state: AppViewState) {
  ...
  const toggleDropdown = () => {
    ...
    if (!state.whatsappDropdownOpen) {
      ...
      if (state.client && state.connected) {
        loadChannels(state as any, true);
      }
    }
  };
  ...
  if (state.channelsSnapshot) {
    ...
    for (const [, ch] of Object.entries(channels)) {
      totalChannels++;
      if ((ch as any)?.connected || (ch as any)?.running) connectedCount++;
    }
  }
...
function renderWaDropdownSettings(state: AppViewState, close: () => void) {
  ...
  const handleDmPolicyChange = async (e: Event) => {
    ...
    const baseHash = state.configSnapshot?.hash ?? undefined;
    try {
                ...
                ${navCollapsed
                  ...
                  : html`
                      ...
                      <span class="sidebar-brand__copy">
                        ...
                        <span class="sidebar-brand__title">FireClaw</span>
                      </span>
                    `}
              </div>
              <button
